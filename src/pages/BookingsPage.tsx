...
const BookingsPage = () => {
  ...
  const activeBookings = section === "personal" ? personalBookings : spaceBookings;
  ...
  const totalCount = personalBookings.length + spaceBookings.length;
      ...
      >
        <div className="flex items-start gap-3">
          <div className={`w-14 h-14 rounded-lg flex items-center justify-center flex-shrink-0 ${
            isSharedSection ? "bg-accent/20" : "bg-muted"
          }`}>
            {isSharedSection 
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-medium text-foreground">
                  {isSharedSection ? `${booking.student_name}` : `Booking #${booking.id.slice(0, 8).toUpperCase()}`}
                </p>
      ...
      <div className="sticky top-0 bg-background border-b border-border px-4 py-3 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{totalCount} total bookings</p>
          </div>
        </div>
      </div>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="font-medium text-foreground mb-1">
              {section === "personal"
                ...
                : (filter === "all" ? "No one has booked your space yet" : `No ${filter} bookings for your space`)
              }
            </h3>
            <p className="text-sm text-muted-foreground mb-4">
              {section === "personal"
                ...
                : (filter === "all"
                    ? "Once someone books your shared space, it will appear here"
                    : "Check back later or try a different filter")
              }
            </p>
            {section === "personal" && filter === "all" && (
